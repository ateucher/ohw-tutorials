...
## CryoCloud/JupyterHub setup

Choose the approximately **15 GB RAM** JupyterHub option when available. After pulling the `OHW26` repository, open a terminal in this folder and install the tested environment once:

```bash
python -m pip install -r requirements.txt
```

Then open the notebook and run it from the top. A GPU is helpful but not required: CPU-only participants use saved results for the training sections by default, while the inference exercises run live. The notebook also includes an opt-in switch for trying the small YOLO fine-tune on a CPU.
