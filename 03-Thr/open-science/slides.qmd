...
## A project-oriented Python layout
...
- All paths in code are relative to this root, e.g.
  `pd.read_csv("data/raw/raw_foofy_data.csv")`.
:::
...
When you open a project (folder), it sets the working directory to the project
root - this is that "stable base"
:::
...
```python
import os
os.chdir("C:/Users/andy/path/that/only/I/have")
...
```python
my_data = pd.read_csv("C:/Users/andy/path/that/only/I/have/data/my_data.csv")
```
:::
...
::: notes
- Package APIs and behaviour change over time; without pinning, a script that
...
pixi run python script.py  # run inside the reproducible env
```
